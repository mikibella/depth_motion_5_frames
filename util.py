...
def load_image(img_file, resize=None, interpolation='linear'):
  ...
  im_data = np.fromstring(gfile.FastGFile(img_file,'rb').read(), np.uint8)
...
def count_parameters(also_print=False):
  ...
  if ckpt is not None:
    ...
    for v in model_vars:
      if v.op.name not in ckpt_var_names:
        ...
        if ckpt_var_shapes[ind] == v.get_shape() and v.op.name != "global_scale_var":
          mapping[v.op.name] = v
