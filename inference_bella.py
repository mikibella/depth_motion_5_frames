...
def _run_inference(output_dir=None,
  ...
  inference_model = model.Model(is_training=False,
                                ...
                                handle_motion=True)
                                #data_dir="C:/Users/bellmi2/Documents/kitti/KITTI-Motion/output_img")

  # #Test of calling model.py
  inference_model.reader = reader.DataReader(inference_model.data_dir, inference_model.batch_size,
                              inference_model.img_height, inference_model.img_width,
                              inference_model.seq_length, 4,
                              inference_model.file_extension,
                              inference_model.random_scale_crop,
                              inference_model.flipping_mode,
                              inference_model.random_color,
                              inference_model.imagenet_norm,
                              False,
                              inference_model.input_file)
  ...
  with sv.managed_session() as sess:
    ...
    if egomotion:
      if inference_mode == INFERENCE_MODE_SINGLE:
        ...
        current_output_handle_objectmotion = None
        for i in range(len(im_files)):
          ...
          if sequence_dir != current_sequence_dir:
            ...
            #objectmotion
            output_filepath_objectmotion = os.path.join(output_dirs[i], 'objectmotion.txt')
            if current_output_handle_objectmotion is not None:
              current_output_handle_objectmotion.close()
            logging.info('Writing objectmotion sequence to %s.', output_filepath_objectmotion)
            current_output_handle_objectmotion = gfile.Open(output_filepath_objectmotion, 'w')
          ...
          # inference_model.build_inference_for_training()
          ...
          current_output_handle_objectmotion.write(
        ...
        for i in range(len(im_files)):
          ...
          if use_masks:
            ...
            #Mask out moving objects to detect the right ego motion
            input_image_stack = mask_image_stack(input_image_stack,
                                                 input_seg_seq)
          ...
          with gfile.Open(output_filepath, file_mode) as current_output_handle:
            current_output_handle.write('triplet: '+str(i) + ' ' + egomotion_1_2 + ' ' +
                                        egomotion_2_3 + '\n')
          written_before.append(output_filepath)

          #Allign Masks and then pass Mask with each Objectmask to the object Motion network
          tmp_aligned = alignment.align(input_seg_seq[0],input_seg_seq[1],input_seg_seq[2])
          object_id_list = unique(tmp_aligned)
          for id in object_id_list:
            if id != 0:
              object_mask = (input_seg_seq == id).astype(int)
              object_mask = object_mask * input_seg_seq
              input_seg_seq_dims =np.concatenate([object_mask[0], object_mask[1],object_mask[2]],axis=2)
              input_seg_seq_dims = np.expand_dims(input_seg_seq_dims, axis=0)
          
              #nur die maske übergeben ?1
              est_objectmotion = inference_model.inference_objectmotion(
                  input_seg_seq_dims, sess)
              ...
              output_filepath_objectmotion = os.path.join(output_dirs[i], 'objectmotion.txt')
              file_mode = 'w' if output_filepath_objectmotion not in written_before else 'a'
              with gfile.Open(output_filepath_objectmotion, file_mode) as current_output_handle_objectmotion:
                current_output_handle_objectmotion.write('triplet: '+str(i) + ' Object ID: '+ str(id) +' ' + objectmotion_1_2 + ' ' +
                                            objectmotion_2_3 + '\n')

              written_before.append(output_filepath_objectmotion)


          
          # (image_stack, image_stack_norm, seg_stack,
          #     intrinsic_mat, intrinsic_mat_inv) = inference_model.reader.read_data()
